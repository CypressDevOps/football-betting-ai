...
import type { KiPredictionService } from "./kiPredictionService";
...
export class RecommendationService {
  constructor(
    private kiService?: KiPredictionService,
    private minOdds: number = 1.8,
    private topK: number = 3
  ) {}

  public async generateRecommendations(matches: Match[]): Promise<Recommendation[]> {
    ...
    for (const m of matches) {
      ...
      // Scores pro Bet-Typ leicht variieren
      const scoreHomeWin = logistic((formHome - formAway) + marketSignal + Math.random() * 0.05);
      const scoreOver25 = logistic((formHome + formAway) / 2 + marketSignal * 0.5 + Math.random() * 0.05);
      const scoreOver35 = logistic((formHome + formAway) / 2 + marketSignal * 0.4 + Math.random() * 0.05);
      const scoreBTTS = logistic((formHome * formAway) + marketSignal * 0.3 + Math.random() * 0.05);
    ...
    // Top K sortieren nach Score & Quote
